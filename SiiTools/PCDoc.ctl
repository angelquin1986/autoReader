...
Private Sub ConfigCols()
    With grd
        .FormatString = "^#|<Código|<Nombre|<Forma|<#Doc|>Valor" & _
                        "|<F.Emisión|>Plazo|<F.Venci.|<Observación|<CodVend.|<Vendedor"
            ...
            For ix = 1 To mobjGNComp.CountPCKardex
                    ...
                    If mobjGNComp.Empresa.VerificarCambioCobroPago(mobjGNComp.PCKardex(ix).id, CodTrans, numtrans, valor) Then
                        If valor <> grd.ValueMatrix(ix, COL_VALOR) Then
                            ...
                            "No se puede Modificar el documento debido a que existen cobros o pagos asignados " & Chr(13) & "con la Transacción: " & CodTrans & "-" & numtrans & "del Cliente: " & mobjGNComp.PCKardex(ix).CodProvCli
                        End If
                        'Exit For
                    End If
                End If
