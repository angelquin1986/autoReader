...
Begin VB.Form frmCierreTransXDevolver 
   Caption         =   "Pendientes x Devolver"
   ClientHeight    =   7845
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   9825
   ...
   Begin VB.Label lblPasos 
      BackColor       =   &H00C0FFFF&
      BorderStyle     =   1  'Fixed Single
      Caption         =   "Pasar trans.Pendientes de Producir"
      Height          =   330
      Left            =   120
      TabIndex        =   15
      Top             =   1500
   ...
   End
End
Attribute VB_Name = "frmCierreTransXDevolver"
...
Public Sub Inicio()
    ...
    Me.Caption = "Transacciones No devueltas"
...
Public Sub InicioProduccion()
    On Error GoTo ErrTrap
    Set mObjCond = New RepCondicion
    Me.Caption = "Transacciones Pendientes Producir"
    Me.tag = "TransPendProducir"
    Me.Show
    Exit Sub
ErrTrap:
    DispErr
...
Private Sub cmdPasos_Click()
    Dim r As Boolean
    Select Case Me.tag
        Case "TransPendProducir"
            If Not frmB_PendxFamilia.InicioPendientesProduccion(mObjCond, Me.tag) Then
                grd.SetFocus
                Exit Sub
            End If
            r = CopiaTrans
...
'    Si existe en el destino, sobreescribe
    ...
    If Len(grd.TextMatrix(fila, COL_CODCLI)) > 0 Then
        ...
        gnDest.IdClienteRef = grd.ValueMatrix(fila, COL_CODCLI)
    End If
    i = 1
    contItems = 0
    While i <= gnOri.CountIVKardex
        ...
        While BandOtro
            If gnOri.IVKardex(i).CodInventario = grd.TextMatrix(fila, COL_CODITEM) Then
                ...
                gnDest.IVKardex(contItems).cantidad = gnOri.IVKardex(i).cantidad
                ...
                gnDest.IVKardex(contItems).orden = gnOri.IVKardex(i).orden
                ...
                gnDest.IVKardex(contItems).bandImprimir = gnOri.IVKardex(i).bandImprimir
                gnDest.IVKardex(contItems).IdICE = gnOri.IVKardex(i).IdICE
                gnDest.IVKardex(contItems).IdPadre = gnOri.IVKardex(i).IdPadre
                gnDest.IVKardex(contItems).bandVer = gnOri.IVKardex(i).bandVer
                gnDest.IVKardex(contItems).idpadresub = gnOri.IVKardex(i).idpadresub
                BandEncontro = True
            End If
...
'                        Fila = 5
    ...
    gnDest.Descripcion = gnDest.Descripcion & " /PENDIENTE DESDE " & gnOri.FechaTrans
...
'            If .IsSelected(i) Then
                ...
                If grd.TextMatrix(i, .Cols - 1) <> MSG_OK Then
                    ultimaFila = i
                    If grd.ValueMatrix(i, 10) > 0 Then
...
'        CondtransFac = Cond & " AND  GNComprobante.CodTrans IN (" & PreparaCadena(.SQLItem) & ") "
        ...
        sql = "SELECT GNComprobante.idTransFuente, GNComprobante.FechaTrans, CodTrans, NumTrans , PCProvCli.CodProvcli, PCProvCli.Nombre, " & _
              ...
              "sum(IVKardex.Cantidad) As Cantidad, GNComprobante.IdCentro, GNCentroCosto.CodCentro,ivb.codbodega "
        ...
        sql = sql & "INTO tmp1  FROM IvInventario" & " INNER JOIN ((GNComprobante LEFT JOIN PCProvCli " & _
             ...
             "INNER JOIN IVKardex  INNER  JOIN IVBODEGA IVB ON IVB.IDBODEGA = IVKARDEX.IDBODEGA ON GNComprobante.TransID = IVKardex.TransID) " & _
             "ON IVInventario.IdInventario = IVKardex.IdInventario "
        sql = sql & " WHERE GNComprobante.Estado<>3 And gncentroCosto.fechafinal is null"
        ...
        sql = sql & "GROUP BY GNComprobante.FechaTrans, CodTrans,NumTrans " & _
             ...
             " gnComprobante.IdCentro, GNCentroCosto.CodCentro,GNComprobante.idTransFuente,ivb.codbodega "
...
'Luego quitamos todos los que tiene saldo cero
        ...
        sql = "Select tmp1.FechaTrans, tmp1.codTrans, tmp1.NumTrans, tmp1.CodCentro, tmp1.CodProvcli, tmp1.Nombre,  " & _
               "tmp1.CodInventario, tmp1.Descripcion, tmp1.CodBodega, abs(tmp1.Cantidad) as cantidad " & _
               "From tmp1 "
'               "Union " & _
...
Private Sub CargaPendientexFamilia()
    ...
    With mObjCond
        ...
        sql = sql & " Into tmp1 "
        sql = sql & " from tmp111 a left join tmp122 b on a.IdCentro=b.IdCentro and a.IdMateria=b.IdMateria"
        mEmpOrigen.EjecutarSQL sql, NumReg 'Priemer SQL Transacciones de salida PTerminado
...
Private Function CopiaTransHormi() As Boolean
    ...
    If grd.FixedRows = grd.Rows Then CargaPendienteHormi 'Carga  trans solo  si la grlla esta vacia
    'Transferir  transaccion una por una
    If MsgBox("Este proceso tardará  algunos minutos " & Chr(13) & " Desea comenzar el proceso de importación?", _
                vbYesNo + vbQuestion) <> vbYes Then Exit Function
        ...
        While BandOtro
            If gnOri.IVKardex(i).CodInventario = grd.TextMatrix(fila, COL_CODITEM) Then
                ...
                If gnDest.GNTrans.IVTipoTrans = "E" Then
                    gnDest.IVKardex(contItems).cantidad = grd.TextMatrix(fila, 10) * -1
                Else
                    gnDest.IVKardex(contItems).cantidad = grd.TextMatrix(fila, 10)
                End If
...
Private Sub VisualizarTexto(ByVal archi As String)
    ...
    If grd.Rows > 1 Then
            ...
            Next i
            grd.ColSort(10) = flexSortGenericAscending
            grd.Select 1, 1, 1, 10
...
Private Sub ConfigCols()
    ...
    grd.Cols = 1
    grd.FormatString = "^#|<Fecha|<CodTrans|<# Trans|<Cod Cliente|<Cliente|<Cod Item|<Descripcion Item|<Cod Bodega|<Fecha Devolucion|<Resultado"
    grd.ColDataType(1) = flexDTDate
    grd.ColDataType(2) = flexDTString
