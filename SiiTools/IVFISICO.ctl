...
Begin VB.UserControl IVFISICO 
   ...
   ScaleHeight     =   2655
   ScaleWidth      =   5460
   Begin VSFlex7Ctl.VSFlexGrid grdTemp 
      Height          =   315
      Left            =   60
      TabIndex        =   1
      Top             =   2220
      ...
      FormatString    =   $"IVFISICO.ctx":0000
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Const COL_DIF = 27      '*** Para calcular diferencia de existentecias
...
'*** Angel 12/Mar/2003 Agregado
Public Property Get DiferenciaExistencia(ByVal ix As Long) As Currency
    If (grd.Rows = grd.FixedRows) Then
        DiferenciaExistencia = 0
    Else
        DiferenciaExistencia = grd.ValueMatrix(ix, COL_DIF)
    End If
End Property
...
Private Sub ConfigCols()
    ...
    s = s & "|<Cod.Item|<Cod.Alterno|<Descripción|>Exist|>Cant. Física|<Unid|>Costo U.|>Costo U.R." & _
            "|>Costo T.|>Costo T.R.|>%Util|>Precio U.|>Precio U.R.|>Precio U+IVA|>Precio T.|>Precio T.R." & _
            "|>P. Total + IVA|>%Dscnt|>%IVA|>IVA|>Diferencia"
    ...
    With grd
        ...
        .ColWidth(COL_DIF) = COLANCHO_CUR           'Diferencia de existencias
        ...
        .ColDataType(COL_DIF) = flexDTCurrency
...
Private Sub ConfigColsVisible()
    ...
    With grd
        .Cols = COL_DIF + 1            '*** MAKOTO 16/dic/00 Agregado
        ...
        .ColHidden(COL_CU) = True 'Not CBool(v And &H80000800)
        .ColHidden(COL_CUR) = True 'Not CBool(v And &H80001000)
        .ColHidden(COL_CT) = True 'Not CBool(v And &H80002000)
        .ColHidden(COL_CTR) = True 'Not CBool(v And &H80004000)
        .ColHidden(COL_UTIL) = True 'Not CBool(v And &H80008000)
        .ColHidden(COL_PU) = True 'Not CBool(v And &H80010000)
        .ColHidden(COL_PUR) = True 'Not CBool(v And &H80020000)
        .ColHidden(COL_PUIVA) = True 'Not CBool(v And &H80040000) '******** jeaa 22-Dic-03
        .ColHidden(COL_PT) = True 'Not CBool(v And &H80080000)
        .ColHidden(COL_PTR) = True ' Not CBool(v And &H80100000)
        .ColHidden(COL_PTIVA) = True 'Not CBool(v And &H80200000) '******** jeaa 22-Dic-03
        .ColHidden(COL_PORDCNT) = True 'Not CBool(v And &H80400000)
        .ColHidden(COL_PORIVA) = True 'Not CBool(v And &H80800000)
        .ColHidden(COL_VALIVA) = True 'Not CBool(v And &H81000000)
        .ColHidden(COL_UNIDAD) = True 'Not CBool(v And &H82000000)   '*** MAKOTO 22/jul/00
        .ColHidden(COL_DIF) = False
...
'        .ColData(COL_CODALT) = CInt(Not (CBool(v2 And &H80000080) Or CBool(v3 And &H80000080)))
        ...
        Else
          .ColData(COL_CODITEM) = CInt(Not (CBool(v2 And &H80000040) Or CBool(v3 And &H80000040)))
          'Carga memoria la pantalla  de busquea
        End If
        ...
        If .ColData(COL_DESC) = 0 Then
          'Set frmBDescripcion = New frmIVgrdBusqueda
          'PreparaComboDescripcion
        End If
...
Private Sub ConfigColsFormato()
    With grd
        ...
        .ColFormat(COL_DIF) = .ColFormat(COL_EXIST)
...
Private Sub grd_AfterEdit(ByVal Row As Long, ByVal col As Long)
    Dim obj As IVKardex, cod As String, i As Long, mover As Boolean
    ...
    With grd
        ...
        Case COL_CODITEM
            ...
            If gConfigIVFisico.BandLineaAuto Then
                'jeaa 13/10/04
                If gConfigIVFisico.BandTotalizarItem Then TotalizarItem
        ...
        Case COL_DESC
            ...
            End If
        Case COL_CANT
            .TextMatrix(Row, COL_DIF) = .ValueMatrix(Row, COL_CANT) - .ValueMatrix(Row, COL_EXIST)
            If gobjMain.EmpresaActual.GNOpcion.IVKTipoDatoDouble Then
...
'''''''******************
        ...
        '***Angel 12/mar/04
        grd.TextMatrix(Row, COL_DIF) = grd.ValueMatrix(Row, COL_CANT) - grd.ValueMatrix(Row, COL_EXIST)
...
Public Sub TotalizarItem()
    ...
    If mobjGNComp.TotalizaItemRepetido Then
        'Actualiza la pantalla
        If gobjMain.EmpresaActual.GNOpcion.IVKTipoDatoDouble Then
            VisualizaDesdeObjetoDou
        Else
            VisualizaDesdeObjeto
        End If
        
        ActualizarDiferenciaExistencia
...
'                    s = s & Abs(ivk.Cantidad) & vbTab & iv.Unidad & vbTab   '*** MAKOTO 22/jul/00
            ...
            If Len(s) > 0 Then          '*** MAKOTO 09/nov/00 para no agregar items de destino en Trans. Bodegas
                ...
                'Cambia de color si la fila no fue contada. Osea fue agregada por el sistema
                If ivk.Nota = NOCONTADO Then
                    .Cell(flexcpForeColor, .Rows - 1, .FixedCols, .Rows - 1, .Cols - 1) = COLOR_NOCONTADO
                End If
            End If
            If mobjGNComp.idTransFuente <> 0 Then   '*** Oliver 26/sep/2003 para tener un respaldos de items Importados
                ItemsImportados(0, i - 1) = ivk.CodInventario
...
Private Sub ActualizarDiferenciaExistencia()
    Dim i As Long
    With grd
        If grd.Rows = grd.FixedRows Then Exit Sub
        For i = 1 To mobjGNComp.CountIVKardex
            .TextMatrix(i, COL_DIF) = .ValueMatrix(i, COL_CANT) - .ValueMatrix(i, COL_EXIST)
        Next i
    End With
End Sub
...
Public Function Grabar() As String
    Dim i As Long, s As String, archi As String
    
    grdTemp.Rows = grdTemp.FixedRows
    With grd
        For i = .FixedRows To .Rows - 1
            If Not (.IsSubtotal(i)) Then
                s = .TextMatrix(i, COL_NUMFILA) & vbTab & _
                    .TextMatrix(i, COL_CODBODEGA) & vbTab & _
                    .TextMatrix(i, COL_CODITEM) & vbTab & _
                    .TextMatrix(i, COL_CODALT) & vbTab & _
                    .TextMatrix(i, COL_DESC) & vbTab & _
                    .TextMatrix(i, COL_EXIST) & vbTab & _
                    .TextMatrix(i, COL_CANT) & vbTab & _
                    .TextMatrix(i, COL_DIF)
                If .Cell(flexcpForeColor, i, COL_DIF, i, COL_DIF) = COLOR_NOCONTADO Then
                    s = s & vbTab & NOCONTADO
                Else
                    s = s & vbTab & ""
                End If
        ...
        Next i
    End With
    archi = Trim$(mobjGNComp.CodTrans) & "_" & Format(Date, "dd-MM-yyyy")
    ExportaTxtConTab grdTemp, archi
    Grabar = archi
End Function
...
'***Este procedimiento va a buscar todos los items que no consten en la lista de la
'***constatación física de inventario
Public Sub CargarItemsNoContados()
    'Crea una tabla tmp con los items que has sido contados fisicamente
    PreparaTmpIVFisico
        
    'Agrega los items que faltan y que tengan existencia diferente de cero
    If gobjMain.EmpresaActual.GNOpcion.IVKTipoDatoDouble Then
        AgregaItemsConExistenciaDou
    Else
        AgregaItemsConExistencia
    End If
...
Private Sub EliminaTmpIVFisico()
    Dim sql As String, rs As Recordset, Num As Long
    
    'Busca la tabla temporal: si esta creada la borra.
    sql = "select * from sysobjects where name='tmpIVFisico'"
    Set rs = gobjMain.EmpresaActual.OpenRecordset(sql)
    If Not (rs.BOF And rs.EOF) Then
        sql = "DROP TABLE tmpIVFisico"
        gobjMain.EmpresaActual.EjecutarSQL sql, Num
    End If
    If Not (rs Is Nothing) Then Set rs = Nothing
End Sub

Private Sub PreparaTmpIVFisico()
    Dim sql As String, Num As Long
    Dim i As Long, cod As String
    
    EliminaTmpIVFisico
    
    'Crea una tabla temporal
    sql = "CREATE TABLE tmpIVFisico (CodInventario Varchar(20) NOT NULL)"
    gobjMain.EmpresaActual.EjecutarSQL sql, Num
    
    'Pasa los items contados fisicamente a la tabla temporal
    With grd
        For i = .FixedRows To .Rows - 1
            If Not (.IsSubtotal(i)) Then
                cod = Trim$(.TextMatrix(i, COL_CODITEM))
                If Len(cod) > 0 Then
                    sql = "INSERT INTO tmpIVFisico (CodInventario) VALUES ('" & cod & "')"
                    gobjMain.EmpresaActual.EjecutarSQL sql, Num
                End If
            End If
        Next i
...
Private Sub AgregaItemsConExistencia()
    Dim sql As String, rs As Recordset
    
    sql = "SELECT IVInventario.CodInventario, IVInventario.Descripcion, " & _
          "IVBodega.CodBodega, SUM(IVKardex.Cantidad) AS Existencia " & _
          "FROM IVInventario INNER JOIN " & _
    ...
    sql = sql & "WHERE  (GNComprobante.FechaTrans <= " & FechaYMD(mobjGNComp.FechaTrans, gobjMain.TipoDB) & ")  AND " & _
     " (IVInventario.BandValida = 1) AND "
    sql = sql & " (GNtrans.AfectaCantidad = 1)  AND "
    sql = sql & " (GNComprobante.Estado <> 3) AND "
    sql = sql & " (IVInventario.BandServicio = 0) AND "
    sql = sql & " (IVInventario.CodInventario NOT IN (SELECT CodInventario FROM tmpIVFisico GROUP BY CodInventario)) "
    
    If grd.ColHidden(COL_CODBODEGA) Then
        If Len(mobjGNComp.GNTrans.CodBodegaPre) > 0 Then
            sql = sql & "AND (IVBodega.CodBodega='" & mobjGNComp.GNTrans.CodBodegaPre & "') "
        End If
    Else
        sql = sql & "AND (IVBodega.CodBodega='" & mobjGNComp.GNTrans.CodBodegaPre & "') "
        '& "' or
        'sql = sql & "IVBodega.CodBodega='" & Mid$(mobjGNComp.GNTrans.CodBodegaPre, 1, Len(mobjGNComp.GNTrans.CodBodegaPre) - 1) & "2') "
    End If
    
    sql = sql & "GROUP BY IVInventario.CodInventario, IVInventario.Descripcion, "
'    If Len(mobjGNComp.GNTrans.CodBodegaPre) > 0 Then
        sql = sql & " IVBodega.CodBodega "
 '   End If
    sql = sql & "  Having Sum(IVKardex.Cantidad) <> 0 " & _
    ...
    If Not (rs.BOF And rs.EOF) Then
        ...
        With mobjGNComp
            Do Until rs.EOF
                ...
                .IVKardex(ix).cantidad = 0   '***rs!Existencia * -1
                .IVKardex(ix).Nota = NOCONTADO
                rs.MoveNext
            Loop
        End With
    End If
...
'***Este procedimiento va a cargar nuevamente el objeto
'***si esque cambio el orden
Public Sub CargaItemsOrdenado()
   Dim i As Integer
   On Error GoTo ErrTrap
    Me.GNComprobante.BorrarIVKardex
'    Me.VisualizaDesdeObjeto
    
    For i = 1 To grd.Rows - 1
        'Pasa al objeto los items seleccionados
        Dim ix As Long
        If Not grd.IsSubtotal(i) Then
            With Me.GNComprobante
                    ix = .AddIVKardex
                    .IVKardex(ix).CodBodega = grd.TextMatrix(i, COL_CODBODEGA)
                    .IVKardex(ix).CodInventario = grd.TextMatrix(i, COL_CODITEM)
                    If gobjMain.EmpresaActual.GNOpcion.IVKTipoDatoDouble Then
                        .IVKardex(ix).cantidadDou = grd.TextMatrix(i, COL_CANT)
                    Else
                        .IVKardex(ix).cantidad = grd.TextMatrix(i, COL_CANT)
                    End If
            End With
        End If
    Next i
...
Public Sub VisualizaDesdeObjetoAjustes()
    Dim ivk As IVKardex, iv As IVinventario, i As Long, s As String, ut As Single, col As Integer, fil As Integer
...
Public Sub CargarItemsAjuste(grd As Object)
    'Crea una tabla tmp con los items que has sido contados fisicamente
    PreparaTmpIVFisico
...
Private Sub AgregaItemsParaAjuste(grd As Object)
    Dim sql As String, rs As Recordset
    Dim i As Long
    Dim ix As Long
    ...
    For i = 1 To grd.Rows - 1
       'Pasa al objeto los items seleccionados
        With mobjGNComp
                ix = .AddIVKardex
                .IVKardex(ix).CodBodega = grd.TextMatrix(i, 3)
                .IVKardex(ix).CodInventario = grd.TextMatrix(i, 1)
                .IVKardex(ix).cantidad = grd.TextMatrix(i, 4)
                .IVKardex(ix).Nota = "AJUSTE"
        End With
    Next i
End Sub


Public Sub VisualizaDesdeObjetoMasDiferencia()
    Dim ivk As IVKardex, iv As IVinventario, i As Long, s As String, ut As Single, col As Integer, fil As Integer
...
'                    s = s & Abs(ivk.Cantidad) & vbTab & iv.Unidad & vbTab   '*** MAKOTO 22/jul/00
                    ...
                    Else
                        's = s & ivk.Cantidad+
                        If grd.ColHidden(COL_CODBODEGA) Then        '*** MAKOTO 15/dic/00
                            s = s & ivk.cantidad + iv.Existencia("") & vbTab     'Suma de todas las bodegas
                        Else
                            s = s & ivk.cantidad + iv.Existencia(ivk.CodBodega) & vbTab
                        End If
...
Public Property Get AjusteExistencia(ByVal ix As Long) As Currency
    If (grd.Rows = grd.FixedRows) Then
        AjusteExistencia = 0
    Else
        AjusteExistencia = grd.ValueMatrix(ix, COL_CANT)
    End If
End Property
...
Private Sub AgregaItemsConExistenciaDou()
    Dim sql As String, rs As Recordset
    sql = "SELECT IVInventario.CodInventario, IVInventario.Descripcion, " & _
          "IVBodega.CodBodega, SUM(IVKardex.CantidadDou) AS Existencia " & _
          "FROM IVInventario INNER JOIN " & _
                  "(IVBodega INNER JOIN " & _
                  "(IVKardex INNER JOIN " & _
                  "(GNtrans  INNER JOIN GNComprobante " & _
...
'    If Len(mobjGNComp.GNTrans.CodBodegaPre) > 0 Then
        sql = sql & " IVBodega.CodBodega "
 '   End If
    sql = sql & "  Having Sum(IVKardex.CantidadDou) <> 0 " & _
                "ORDER BY SUM(IVKardex.CantidadDou)"
    Set rs = gobjMain.EmpresaActual.OpenRecordset(sql)
    If Not (rs.BOF And rs.EOF) Then
        'Pasa al objeto los items seleccionados
        Dim ix As Long
        With mobjGNComp
            Do Until rs.EOF
                ...
                .IVKardex(ix).CodBodega = rs!CodBodega
                .IVKardex(ix).CodInventario = rs!CodInventario
                .IVKardex(ix).cantidadDou = 0   '***rs!Existencia * -1
                .IVKardex(ix).Nota = NOCONTADO
                rs.MoveNext
            Loop
        End With
