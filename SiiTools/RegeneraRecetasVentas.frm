...
Begin VB.Form frmRegeneraRecetasVentas 
   ...
   Begin VB.Label Label2 
      Caption         =   "Lista de Items faltantes"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmRegeneraRecetasVentas"
...
ErrTrap:
    ...
    Exit Sub
End Sub
Public Sub InicioCorrigeCantidades()
    Dim i As Integer
    On Error GoTo ErrTrap
    Me.Show
    Me.ZOrder
    ...
    Me.Caption = "Correccion de Cantidades"
    Exit Sub
ErrTrap:
    DispErr
    Unload Me
...
'    fcbTrans.SetData gobjMain.GrupoActual.PermisoActual.ListaTrans(False, "IV")
    ...
    Next i
        ...
        'jeaa 25/09/206
        If Len(gobjMain.EmpresaActual.GNOpcion.ObtenerValor("TransparaReprocesoRecetaItem")) > 0 Then
            s = gobjMain.EmpresaActual.GNOpcion.ObtenerValor("TransparaReprocesoRecetaItem")
...
Private Sub cmdBuscar_Click()
    ...
    With gobjMain.objCondicion
        ...
        .CodGrupo = fcbGrupoDesde.KeyText
        .CodItem1 = fcbDesde2.KeyText
        s = PreparaTransParaGnopcion(.CodTrans)
        gobjMain.EmpresaActual.GNOpcion.AsignarValor "TransparaReprocesoRecetaItem", s
        gobjMain.EmpresaActual.GNOpcion.Grabar
        
    End With
    Set obj = gobjMain.EmpresaActual.ConsGNTransRecetaxItem(True)
    If Not obj.EOF Then
        v = MiGetRows(obj)
...
Private Sub ConfigCols()
    ...
    End With
    With grdReceta
        .FormatString = "^#|ID|Trans|idInventario|<Cod.Inv|<Descripcion|>Cantidad|>Tipo|>IdPadre|<Resultado|>Cant.Real|>Id|>PU|>Ast "
        GNPoneNumFila grd, False
        .AutoSize 0, .Cols - 1
        .ColWidth(1) = 700
        .ColWidth(2) = 1500
    ...
    End With
End Sub
Private Sub cmdBuscarCantidad_Click()
    Dim v As Variant, obj As Object, s As String
    On Error GoTo ErrTrap
'    If Len(fcbGrupoDesde.KeyText) = 0 Then
'        MsgBox "deberia escoger el grupo para aplicar"
...
'    End If
    ...
    With gobjMain.objCondicion
        ...
        gobjMain.EmpresaActual.GNOpcion.Grabar
    End With
    Set obj = gobjMain.EmpresaActual.ConsGNTrans
    If Not obj.EOF Then
        v = MiGetRows(obj)
        grd.Redraw = flexRDNone
        grd.LoadArray v
...
'    grd.Move 0, grd.Top, Me.ScaleWidth, (Me.ScaleHeight - grd.Top - pic1.Height - 80)
    ...
    grdReceta.Move grd.Width, grd.Top, Me.ScaleWidth / 2, (Me.ScaleHeight - grd.Top - pic1.Height - 80)
...
Private Sub cargaItemsKardex(ByRef gnc As GNComprobante, ByVal i As Long, Optional coditem As String)
    ...
    For j = 1 To gnc.CountIVKardex
        If gnc.IVKardex(j).CodInventario = coditem Then
            
                    Set item = gnc.Empresa.RecuperaIVInventario(coditem)
                    If Abs(gnc.IVKardex(j).PrecioTotal) > 0 Then IdPadreT = item.RecuperaId(coditem)
            
                    If item.Tipo = Preparacion Then
                        RecursivoReceta gnc, j, gnc.IVKardex(j).CodInventario, grd.ValueMatrix(i, 16)
                    End If
...
Private Function CorrigeItemsFaltantes()  'ByVal idInven As Long, ByVal cant As Currency, ByVal idPadres As Long, gnc As GNComprobante) As Boolean
    ...
    For i = 1 To grdReceta.Rows - 1
     ...
     x = grdReceta.CellTop
        ...
        If grdReceta.TextMatrix(i, 9) <> "OK." Then
            tid = grdReceta.ValueMatrix(i, 1)
            cant = grdReceta.ValueMatrix(i, 6) * -1
            grdReceta.Refresh
            'Recupera la transaccion
            Set gnc = gobjMain.EmpresaActual.RecuperaGNComprobante(tid)
            Set item = gnc.Empresa.RecuperaIVInventario(grdReceta.ValueMatrix(i, 3))
            sql = "select max(orden) as orden from ivkardex where transid = " & tid
            Set rs = gnc.Empresa.OpenRecordset(sql)
            If rs.RecordCount > 0 Then orden = rs!orden + 1
            costo = item.CostoDouble2(Date, 0, 0, Time)
            ...
            If gnc.GNTrans.CodPantalla = "IVPVTS" And item.CodGrupo(1) = "00004" Then
                ...
                sql = sql & " Values(" & grdReceta.ValueMatrix(i, 1) & ", " & grdReceta.ValueMatrix(i, 3) & ", " & IdBod & ", " & cant & ", " & costo * cant & ", " & costo * cant & " "
            Else
                sql = sql & " Values(" & grdReceta.ValueMatrix(i, 1) & ", " & grdReceta.ValueMatrix(i, 3) & ", " & grdReceta.ValueMatrix(i, 7) & ", " & cant & ", " & costo * cant & ", " & costo * cant & " "
            End If
            sql = sql & ", 0 ,0,0, " & item.PorcentajeIVA & ", " & orden & ",'R', 0, 0, 0, 0, 0, 0," & grdReceta.ValueMatrix(i, 8) & ", 0)"
            gobjMain.EmpresaActual.EjecutarSQL sql, 1
        End If
        grdReceta.TextMatrix(i, 9) = "OK."
...
Private Sub CargaListaItems()
    Dim numGrupo As Integer, v() As Variant
    Dim sql  As String, rs As Recordset, cond As String
    
    fcbGrupoDesde.Clear
    fcbGrupoDesde.SetData gobjMain.EmpresaActual.ListaIVGrupo(1, False, False) 'TOMA SOLO EL GRUPO 1
    
    fcbDesde2.Clear
    sql = "SELECT CodInventario, IVInventario.Descripcion FROM IVInventario  where tipo = 4"
