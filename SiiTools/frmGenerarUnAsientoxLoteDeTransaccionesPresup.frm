...
Begin VB.Form frmGenerarUnAsientoxLoteDeTransaccionesPresup 
   ...
   Begin TabDlg.SSTab sst1 
      ...
      TabPicture(0)   =   "frmGenerarUnAsientoxLoteDeTransaccionesPresup.frx":0000
      ...
      TabPicture(1)   =   "frmGenerarUnAsientoxLoteDeTransaccionesPresup.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "fraEnc"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "Asiento"
      Tab(1).Control(1).Enabled=   0   'False
      Tab(1).ControlCount=   2
      Begin SiiToolsA.Presupuesto Asiento 
         Height          =   2115
         Left            =   -74880
         TabIndex        =   34
         Top             =   2040
   ...
   End
End
Attribute VB_Name = "frmGenerarUnAsientoxLoteDeTransaccionesPresup"
...
'    If MsgBox(s, vbYesNo + vbQuestion) <> vbYes Then Exit Function
    ...
    If Not mobjGNComp Is Nothing Then
        mobjGNComp.Generar1AsientoxLotePresupuesto
...
Private Sub PoneDescripcion()
    Dim gnt As GNTrans, fdesde As Date, fhasta As Date, s As String
    'pone descripcion en la transaccion
    s = PreparaTransParaGnopcion(gobjMain.objCondicion.CodTrans)
'    Set gnt = gobjMain.EmpresaActual.RecuperaGNTrans(s)
'    If Not gnt Is Nothing Then
        fdesde = (dtpFecha1.value + dtpHora1.value)
        fhasta = (dtpFecha2.value + dtpHora2.value)
        txtDescripcion.Text = "Resumen de Asiento de Presupuesto desde " & fdesde & " hasta " & fhasta
'    Else
