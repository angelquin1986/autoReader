...
Begin VB.Form frmRegeneraAsiento 
   ...
   End
End
Attribute VB_Name = "frmRegeneraAsiento"
...
Private Sub cmdAceptar_Click()
    ...
    If Me.Caption = "Verificacion Asientos Duplicados" Then
        BorraAsiento
    Else
...
Private Sub ConfigCols()
    With grd
        ...
        .ColWidth(COL_FECHA) = 1000
        .ColWidth(COL_CODASIENTO) = 1000
        .ColWidth(COL_CODTRANS) = 1000
...
Public Sub InicioAsientoDuplicado()
    Dim i As Integer
    On Error GoTo ErrTrap
    ...
    cmdBuscarTrans.Visible = True
    grd.Top = cmdBuscarTrans.Top + 500
    Me.Caption = "Verificacion Asientos Duplicados"
'    grd.Height = 4000
'    grdTrans.Width = grd.Width
 '   grdTrans.Left = grd.Left
  '  grdTrans.Height = 5000
   ' grdTrans.Top = grd.Top + 4100
...
Private Sub cmdBuscarTrans_Click()
    Dim v As Variant, obj As Object, i As Integer, rs   As Recordset, j As Integer, cont As Integer
    On Error GoTo ErrTrap
    ...
    With gobjMain.objCondicion
        ...
        End If
    End With
    Set obj = gobjMain.EmpresaActual.ConsGNTrans2BuscaAsientoDuplicado(True) 'Ascendente     '*** MAKOTO 20/oct/00
    If Not obj.EOF Then
        v = MiGetRows(obj)
        
        grdTrans.Redraw = flexRDNone
        grdTrans.LoadArray v
        ConfigCols
        grdTrans.Redraw = flexRDDirect
    Else
        grdTrans.Rows = grd.FixedRows
        ConfigCols
    End If
    ...
    End If
        ...
        For i = 1 To grdTrans.Rows - 1
            Set rs = gobjMain.EmpresaActual.ConsGNTrans2AsientoDuplicado(grdTrans.TextMatrix(i, 2))
            rs.MoveFirst
            For j = 1 To rs.RecordCount
                grd.AddItem cont & vbTab & rs.Fields(0) & vbTab & rs.Fields(1) & vbTab & rs.Fields(2) & vbTab & rs.Fields(3) & vbTab & rs.Fields(4) & vbTab & rs.Fields(5) & vbTab & rs.Fields(6)
                cont = cont + 1
                rs.MoveNext
            Next j
        Next i
    
    grd.ColHidden(COL_CODASIENTO) = False
...
Private Function BorraAsiento() As Boolean
    Dim s As String, tid As Long, i As Long, x As Single
    Dim gnc As GNComprobante, cambiado As Boolean, TransID  As Long
...
'        x = grd.CellTop                 'Para visualizar la celda actual
        ...
        gobjMain.EmpresaActual.BorraAsientoDuplicado grd.TextMatrix(i, COL_CODASIENTO)
        grd.TextMatrix(i, COL_CODASIENTO) = "0"
