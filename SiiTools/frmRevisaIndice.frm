...
Begin VB.Form frmRevisaIndice 
   ...
   Begin VB.TextBox txttransrevisar 
      Height          =   375
      Left            =   7560
      TabIndex        =   38
      Top             =   3120
      Width           =   1575
   End
   Begin VB.TextBox txttransFuente 
      Height          =   375
      Left            =   7380
      TabIndex        =   36
      Top             =   2580
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VSFlex7LCtl.VSFlexGrid grd 
         ...
         FormatString    =   $"frmRevisaIndice.frx":0000
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   -1  'True
         MergeCells      =   0
      ...
      End
      Begin VSFlex7LCtl.VSFlexGrid grd1 
         Height          =   5475
         Left            =   7020
         TabIndex        =   35
         Top             =   60
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmRevisaIndice.frx":0063
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   -1  'True
         MergeCells      =   0
   ...
   Begin MSComCtl2.DTPicker dtpFechaCorte 
      ...
      CurrentDate     =   36781
   End
   Begin FlexComboProy.FlexCombo fcbTrans_F 
      Height          =   315
      Left            =   7200
      TabIndex        =   40
      Top             =   300
      ...
      EndProperty
   End
   Begin FlexComboProy.FlexCombo fcbTrans_R 
      Height          =   315
      Left            =   7200
      TabIndex        =   41
      Top             =   1080
   ...
   End
End
Attribute VB_Name = "frmRevisaIndice"
...
Public Sub Inicio()
    ...
    fcbTrans_F.SetData gobjMain.EmpresaActual.ListaGNTrans("IV", False, False)
    fcbTrans_R.SetData gobjMain.EmpresaActual.ListaGNTrans("IV", False, False)
    Me.Show
    Exit Sub
ErrTrap:
    DispErr
...
Private Function GenerarAsientoCierre() As Boolean
    ...
    sql = "SELECT ct.CodCuenta, " & _
          ...
          "WHERE (gc.Estado IN (" & ESTADO_APROBADO & ", " & ESTADO_DESPACHADO & ")) AND " & _
...
'    n = CopiarTabla("GNVersion", "Registro de versiones")
    ...
    '***Agregado. 12/08/08. jeaa. Tabla descto pot item x num pagos
    n = CopiarTabla("DescNumPagIVG", "Catálogo de Desc NumPago-Cliente")
...
Private Function CopiaTrans() As Boolean
    ...
    With grd
        ...
        For i = .FixedRows To .Rows - 1
            ...
            sql = " update " & "[" & Trim$(txtDestinoBD.Text) & "].dbo.gncomprobante set IdTransFuente= "
            sql = sql & " (select transid from " & "[" & Trim$(txtDestinoBD.Text) & "].dbo.gncomprobante "
            sql = sql & " where codtrans='" & grd.TextMatrix(i, 4) & "' and numtrans=" & .TextMatrix(i, 5) & ")"
            sql = sql & "  where codtrans='" & grd.TextMatrix(i, 1) & "' and numtrans=" & .TextMatrix(i, 2)
...
sql = " select gnc.CodTrans, gnc.NumTrans, gnc.IdTransFuente,"
sql = sql & " gnc1.CodTrans , gnc1.numtrans, gnc.TransID"
sql = sql & " from " & "[" & Trim$(lblOrigenBD.Caption) & "].dbo.gncomprobante gnc"
sql = sql & " left join " & "[" & Trim$(lblOrigenBD.Caption) & "].dbo.gncomprobante gnc1 on gnc1.transid=gnc.IdTransFuente"
sql = sql & " where gnc.codtrans='" & fcbTrans_R.KeyText & " ' "
...
sql = " select gnc.CodTrans, gnc.NumTrans, gnc.IdTransFuente,"
sql = sql & " gnc1.CodTrans , gnc1.numtrans, gnc.TransID"
sql = sql & " from " & "[" & Trim$(txtDestinoBD.Text) & "].dbo.gncomprobante gnc"
sql = sql & " left join " & "[" & Trim$(txtDestinoBD.Text) & "].dbo.gncomprobante gnc1 on gnc1.transid=gnc.IdTransFuente"
sql = sql & " where gnc.codtrans='" & fcbTrans_R.KeyText & " ' "
...
' 10/12/2004  antes estaba el orden tambien opor codtrans, y numtrans
    ...
    If Not rs.EOF Then
        ...
        With grd1
            ...
            .FormatString = "^#|<Fecha|<CodTrans|<NumTrans|<Descripción|<Resultado"
            GNPoneNumFila grd, False
            AsignarTituloAColKey grd1            'Para usar ColIndex
            AjustarAutoSize grd1, 0, -1, 3000     'Ajusta automáticamente ancho de cols.
            If .ColWidth(.ColIndex("Descripción")) > 1400 Then .ColWidth(.ColIndex("Descripción")) = 1400
            .ColWidth(.ColIndex("Resultado")) = 1600
            'Tipo de datos
            .ColDataType(.ColIndex("Fecha")) = flexDTDate
