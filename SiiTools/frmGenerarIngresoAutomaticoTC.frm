...
Begin VB.Form frmGenerarIngresoAutomaticoTC 
   ...
   End
End
Attribute VB_Name = "frmGenerarIngresoAutomaticoTC"
...
Private YaImprimioIng As Boolean
...
Private Sub CargaTrans()
    ...
    fcbTrans.SetData gobjMain.GrupoActual.PermisoActual.ListaTrans(False, "TS")
    ...
    v = gobjMain.GrupoActual.PermisoActual.ListaTransxTipoTransNew(False, "IV", "2", "18")
...
Private Sub cmdAceptar_Click()
    If Not mProcesando Then
        ...
        If Me.tag = "CruceTarjetas" Or Me.tag = "CruceIVTarjetas" Then
            If Len(fcbFormaCobro.KeyText) = 0 Then
                MsgBox "No hay ningúna Forma de Cobro no se podrá procesar."
                fcbFormaCobro.SetFocus
                Exit Sub
            End If
        End If
        ...
        If Me.tag = "JEP" Then
            If Len(fcbTrans.KeyText) = 0 Then
                MsgBox "No hay ningúna Transaccion de Ingreso Seleccionada."
                fcbTrans.SetFocus
                Exit Sub
            End If
            If Len(fcbBanco.KeyText) = 0 Then
                MsgBox "No hay ningúnn Banco Seleccionado"
                fcbBanco.SetFocus
                Exit Sub
            End If
    ...
    If Me.tag = "Pago" Then
        If Len(fcbBanco.KeyText) = 0 Then
            MsgBox "Falta seleccionar el Banco"
            fcbBanco.SetFocus
            Exit Sub
        Else
            If EgresoAuto(True, False) Then
            ...
            End If
        End If
    ElseIf Me.tag = "CruceTarjetas" Then
        If CruceTarjetas(True, False) Then
            cmdAceptar.Enabled = True
            cmdAceptar.SetFocus
            mVerificado = True
            cmdAsiento.Enabled = False
    ...
    ElseIf Me.tag = "CruceIVTarjetas" Then
        ...
        Else
            ...
            cmdAsiento.Enabled = True
        End If
    ElseIf Me.tag = "JEP" Then
        If CobroJEP(True, False) Then
            cmdAceptar.Enabled = True
            cmdAceptar.SetFocus
            mVerificado = True
            cmdAsiento.Enabled = False
...
''''        End If
    ...
    grd.Rows = 1
    If Me.tag = "Pago" Then
        Set obj = gobjMain.EmpresaActual.ConsGNTransEgresoAuto(chkAgrupaProv.value = vbChecked, True) 'Ascendente
    ElseIf Me.tag = "CruceTarjetas" Then
        Set obj = gobjMain.EmpresaActual.ConsGNTransCruceTarjeta(chkAgrupaProv.value = vbChecked, True) 'Ascendente
    ElseIf Me.tag = "CruceIVTarjetas" Then
        Set obj = gobjMain.EmpresaActual.ConsGNTransCruceIVTarjeta(chkAgrupaProv.value = vbChecked, True) 'Ascendente
    
    Else
        Set obj = gobjMain.EmpresaActual.ConsGNTransIngresoAutoNew(chkAgrupaProv.value = vbChecked, True) 'Ascendente
    End If
...
Private Sub ConfigCols()
    ...
    With grd
        ...
        Select Case Me.tag
            Case "Pago"
                .FormatString = "^#|tid|<Fecha Trans|<id|<Trans|<#|<#Ref.|<Nombre|<Fecha Venci|<Forma Pago|>Valor|^Autoriza|>Valor Pago|<Resultado|tidIn"
            Case "CruceTarjetas"
                .FormatString = "^#|tid|<Fecha Trans|<id|<Trans|<#|<#Ref.|<Nombre|<Tarjeta|<CodBanco|<Banco|<Auto|>Valor Pago|<Resultado|tidIn"
            Case "CruceIVTarjetas"
                .FormatString = "^#|tid|<Fecha Trans|<id|<Trans|<#|<#Ref.|<Nombre|<Tarjeta|<CodBanco|<Banco|<Auto|>Valor Pago|<Resultado|tidIn"
            Case "JEP"
                .FormatString = "^#|<ReferenciaSobre|<Estado_Sobre|<Fecha_Inicio_Proceso|<Fecha_Vencimiento_Proceso|>ContraPartida|<Referencia|<Moneda|<Nombre Alumna|>Valor Factura|<TipoPago|<TipoId_Cliente|>NumeroId_Cliente|<Estado_Proceso|>Id_Contrato|>Id_Sobre|Id_Item|<PaisBancoCuenta|<Eliminado|<Canal|<Medio|<NumeroDocumento|<Horario|<Mensaje|<Oficina|<FechaProcesoDate|<FechaProceso|<Fecha y Hora Cobro|>Valor Cobro|<Estado_Impresion|<FormaPago|<Tabla|<Pais |<Banco|<Transaccion Cobrada|>Secuencial_Cobro|<Numero_Comprobante|<NumeroCuenta|<No_Documento|<Tipo_Cuenta|<Numero_Cuenta|<CondicionProceso|>Referencia Cobro|<Numero_SRI|<Direccion_Cliente|<comision|<Resultado........................................................|tidIn"
            
            Case Else
                .FormatString = "^#|tid|<Fecha Trans|<id|<Trans|<#|<#Ref.|<Nombre|<Descripción|<Forma Pago|<Cod.Forma|<Auto|>Valor Pago|<Resultado|tidIn"
        End Select
...
Private Sub CargaTransPago()
    ...
    lstTrans.Clear
    v = gobjMain.GrupoActual.PermisoActual.ListaTransxTipoTransNew(False, "", "1")
    For i = LBound(v, 2) To UBound(v, 2)
        lstTrans.AddItem v(0, i)        '& " " & v(1, i)
    Next i
...
Public Sub InicioImportar(ByRef codBanco As String)
    Dim i As Integer
    On Error GoTo ErrTrap
    Me.tag = codBanco
    cmdBuscar.Visible = False
    cmdCargar.Visible = True
    cmdCargar.Left = cmdBuscar.Left
    PicForma.Visible = False
    lblBanco.Visible = True
    fcbBanco.Visible = True
    fcbBanco.TabStop = True
    lblnumche.Visible = False
    lblSal.Visible = False
    lblsaldoBanco.Visible = False
    lblNumCheque.Visible = False
    ...
    fcbTrans.KeyText = gobjMain.EmpresaActual.GNOpcion.ObtenerValor("Trans_ImportarIngresoAuto")
    fcbBanco.KeyText = gobjMain.EmpresaActual.GNOpcion.ObtenerValor("Banco_ImportarIngresoAuto")
...
'        .Filter = "Texto (Separado por coma)|*.txt|Excel 97(XLS)|*.xls"
        .Filter = "Texto (Separado por tabuladores *.txt)|*.txt|Todos *.*|*.*"
        .flags = cdlOFNFileMustExist
        cad = gobjMain.EmpresaActual.GNOpcion.ObtenerValor("Ruta_ImportarIngresoAuto")
        .filename = cad
...
' poner numero
'     grd.SubTotal flexSTSum, 25, 9, , grd.GridColor, vbBlack, , "Subtotal", 25, True
'     grd.SubTotal flexSTSum, 25, 28, , grd.GridColor, vbBlack, , "Subtotal", 25, True
    GNPoneNumFila grd, False
    grd.Redraw = flexRDDirect
    AjustarAutoSize grd, -1, -1
    For i = 1 To grd.Cols - 1
        grd.TextMatrix(2, i) = i
    Next i
...
Private Function CobroJEP(ByVal bandVerificar As Boolean, BandTodo As Boolean) As Boolean
    Dim s As String, tid As Long, i As Long, x As Single, j As Integer, filaSubTotal As Long
    Dim gnc As GNComprobante, cambiado As Boolean, TransGen As String
    Dim FilaIni As Long, FilaFin As Long, k As Long
    ...
    If Not bandVerificar Then
        'Confirma la actualización
        s = "Este proceso creará un Ingreso de Tesorería  de la transacción seleccionada." & vbCr & vbCr
        s = s & "Está seguro que desea proceder?"
        If MsgBox(s, vbYesNo + vbQuestion) <> vbYes Then Exit Function
    End If
    ...
    gobjMain.EmpresaActual.GNOpcion.AsignarValor "Ruta_ImportarIngresoAuto", dlg1.filename
    gobjMain.EmpresaActual.GNOpcion.AsignarValor "Trans_ImportarIngresoAuto", fcbTrans.KeyText
    gobjMain.EmpresaActual.GNOpcion.AsignarValor "Banco_ImportarIngresoAuto", fcbBanco.KeyText
    gobjMain.EmpresaActual.GNOpcion.Grabar


    For i = 6 To grd.Rows - 1
        DoEvents
        If mCancelado Then
            MsgBox "El proceso fue cancelado.", vbInformation
            Exit For
        ...
        If Not grd.IsSubtotal(i) Then
            ...
            tid = grd.ValueMatrix(i, COL_TID)
            grd.TextMatrix(i, 46) = "Procesando  ..."
            grd.Refresh

            'crea la transaccion

            If Not VerificaIngreso(grd.TextMatrix(i, 42)) Then
                'GrabarTransIngresoAutoJEP i
                GrabarTransIngresoAutoJEPNew i
            Else
                grd.TextMatrix(i, 46) = "Cobro ya Ingresado"
            End If
...
Private Function GrabarTransIngresoAutoJEP(ByVal fila As Long) As Boolean
    Dim Imprime As Boolean, i As Long, ix As Long, orden1 As Integer, orden2 As Integer
    Dim pc As PCProvCli, Cadena As String, obser As String, ivb As IVBanco, cadenabanco As String
    Dim tsf As TSFormaCobroPago, gc As GNComprobante, v As Variant, CodTrans As String, numtrans As Long
    Dim fecha As String, gar As PCProvCli, idfactura As Long, rs As Recordset, rspc As Recordset, saldo As Currency
    On Error GoTo ErrTrap
    orden1 = 1
    orden2 = 1
    ...
    If CreaComprobanteIngresoAuto(i) Then
        ...
        v = Split(grd.TextMatrix(fila, 34), " ")
        CodTrans = v(0)
        numtrans = Val(v(1))
        If numtrans <> 0 Then
            Set gc = gobjMain.EmpresaActual.RecuperaGNComprobante(0, CodTrans, numtrans)
        Else
            Set rspc = gobjMain.EmpresaActual.RecuperarPersonaxNombre(grd.TextMatrix(fila, 8))
            If rspc.RecordCount > 0 Then
                
                gobjMain.objCondicion.fecha2 = DateAdd("yyyy", 1, Date)
                gobjMain.objCondicion.PorCobrar = True
                gobjMain.objCondicion.Proveedor = False
                gobjMain.objCondicion.Cliente = True
                gobjMain.objCondicion.CodPC1 = rspc.Fields(1)
                gobjMain.objCondicion.CodPC2 = rspc.Fields(1)
                Set rs = gobjMain.EmpresaActual.ConsTSPorCobrarPagarGarantes
                If rs.RecordCount > 0 Then
                    For i = 0 To rs.RecordCount - 1
                        If rs.Fields(11) = grd.ValueMatrix(fila, 28) Then
                            Set gc = gobjMain.EmpresaActual.RecuperaGNComprobante(rs.Fields(3))
                            Exit For
                        Else
                            rs.MoveNext
                        End If
        ...
        fecha = grd.TextMatrix(fila, 27)
        mobjGNCompAux.FechaTrans = Mid$(fecha, 1, 2) & "/" & Mid$(fecha, 4, 2) & "/" & Mid$(fecha, 7, 4)
        If Len(fecha) = 22 Then
            mobjGNCompAux.HoraTrans = Mid$(fecha, 12, 2) & ":" & Mid$(fecha, 15, 2) & ":" & Mid$(fecha, 18, 2)
        ElseIf Len(fecha) = 21 Then
            mobjGNCompAux.HoraTrans = "0" & Mid$(fecha, 12, 1) & ":" & Mid$(fecha, 14, 2) & ":" & Mid$(fecha, 17, 2)
        End If
            
        Set pc = mobjGNCompAux.Empresa.RecuperaPCProvCli(gc.CodClienteRef)
        
        mobjGNCompAux.codUsuario = gobjMain.UsuarioActual.codUsuario
        mobjGNCompAux.numDocRef = grd.TextMatrix(fila, 42)
        mobjGNCompAux.CodMoneda = gc.CodMoneda
        
        Set gar = mobjGNCompAux.Empresa.RecuperaPCProvCli(Right("0000000" & grd.TextMatrix(fila, 5), 7))
        
        Set gar = mobjGNCompAux.Empresa.RecuperaPCProvCli(Right("0000000" & grd.TextMatrix(fila, 5), 7))
        If gar Is Nothing Then
            Set gar = mobjGNCompAux.Empresa.RecuperaPCProvCli(Right("000000" & grd.TextMatrix(fila, 5), 6))
        End If
        If Not gar Is Nothing Then
            Select Case gar.NumEmiteFactura
                Case 1: idfactura = gar.IdPadre
                Case 2: idfactura = gar.IdMadre
                Case 3: idfactura = gar.IdOtro
            End Select
            mobjGNCompAux.CodGaranteRef = gar.CodProvCli

            mobjGNCompAux.Descripcion = "Por Pago en la: " & fcbBanco.KeyText & ", estudiante: " & gar.CodProvCli & "-" & gar.nombre
        End If
        
        mobjGNCompAux.nombre = gc.nombre
        mobjGNCompAux.CodClienteRef = gc.CodClienteRef
        mobjGNCompAux.CodVendedor = gc.CodVendedor
'        mobjGNCompAux.Descripcion = "Por Pago en la: " & fcbBanco.KeyText & ", estudiante: " & grd.TextMatrix(fila, 8)
        
        
                'PAGO
                ix = mobjGNCompAux.AddPCKardex
                mobjGNCompAux.PCKardex(ix).Haber = grd.ValueMatrix(fila, 28)
                mobjGNCompAux.PCKardex(ix).CodProvCli = gc.CodClienteRef
                mobjGNCompAux.PCKardex(ix).codforma = gc.PCKardex(1).codforma
                mobjGNCompAux.PCKardex(ix).idAsignado = gc.PCKardex(1).id
                mobjGNCompAux.PCKardex(ix).NumLetra = gc.CodTrans & " " & gc.numtrans
                mobjGNCompAux.PCKardex(ix).FechaEmision = gc.FechaTrans
                mobjGNCompAux.PCKardex(ix).CodVendedor = gc.CodVendedor
                mobjGNCompAux.PCKardex(ix).Orden = 1

                'BANCO
                ix = mobjGNCompAux.AddTSKardex
                mobjGNCompAux.TSKardex(ix).codBanco = fcbBanco.KeyText
                mobjGNCompAux.TSKardex(ix).Debe = grd.ValueMatrix(fila, 28)
                mobjGNCompAux.TSKardex(ix).CodTipoDoc = "EF-I"
                mobjGNCompAux.TSKardex(ix).numdoc = grd.ValueMatrix(fila, 42)
                mobjGNCompAux.TSKardex(ix).FechaEmision = mobjGNCompAux.FechaTrans
                mobjGNCompAux.TSKardex(ix).FechaVenci = mobjGNCompAux.FechaTrans
'                mobjGNCompAux.tsKardex(ix).Observacion = mobjGNCompAux.FechaTrans
                
                mobjGNCompAux.TSKardex(ix).Orden = 1
...
'            sst1.Tab = 0
        ...
        If Not YaImprimioIng Then
            Select Case mobjGNCompAux.GNTrans.ImprimeComprobante
    ...
    '    Me.caption = "Transacción " & mobjGNCompAux.codTrans & " " & mobjGNCompAux.NumTrans
        'Me.Caption = mobjGNCompAux.codtrans & " " & mobjGNCompAux.numtrans
        grd.TextMatrix(fila, 46) = mobjGNCompAux.CodTrans & " " & mobjGNCompAux.numtrans
        grd.TextMatrix(fila, 47) = mobjGNCompAux.TransID
        GrabarTransIngresoAutoJEP = True
        Set ivb = Nothing
        Set pc = Nothing
    End If
    Exit Function
ErrTrap:
    MensajeStatus
    grd.TextMatrix(fila, 46) = "Falló Proceso"
    Select Case Err.Number
    Case ERR_DESCUADRADO, ERR_INTEGRIDAD
        'Si es que el usuario seleccionó 'No' en el cuadro de dialogo,
        'No hace nada
...
Private Function CreaComprobanteIngresoAutoJEP(ByRef Num As Long) As Boolean
    Dim v As Currency, tsf As TSFormaCobroPago
    Dim i As Long
        CreaComprobanteIngresoAutoJEP = False
        Set mobjGNCompAux = gobjMain.EmpresaActual.CreaGNComprobante(fcbTrans.KeyText)
        CreaComprobanteIngresoAutoJEP = True
End Function
...
Private Sub ImprimirIngreso(Directo)
    On Error GoTo ErrTrap
    
'    If Not Directo Then
'        If Not (Me Is fMain.ActiveForm) Then Exit Sub
'    End If
    
'    If Not ImprimeTrans(mobjGNCompAux, mobjImp) Then       '*** MAKOTO 11/nov/00
...
Public Function VerificaIngreso(ref As String) As Boolean
Dim sql As String, rs As Recordset, i As Integer
    VerificaIngreso = False
    
        sql = "select transid from gncomprobante  where estado<>3 and codtrans = '" & fcbTrans.KeyText & "' and numdocref='" & ref & "'"
        Set rs = gobjMain.EmpresaActual.OpenRecordset(sql)
        If rs.RecordCount > 0 Then
            VerificaIngreso = True
        End If

End Function

Private Function GrabarTransIngresoAutoJEPNew(ByVal fila As Long) As Boolean
    Dim Imprime As Boolean, i As Long, ix As Long, orden1 As Integer, orden2 As Integer
    Dim pc As PCProvCli, Cadena As String, obser As String, ivb As IVBanco, cadenabanco As String
    Dim tsf As TSFormaCobroPago, gc As GNComprobante, v As Variant, CodTrans As String, numtrans As Long
    Dim fecha As String, gar As PCProvCli, idfactura As Long, rs As Recordset, rspc As Recordset, saldo As Currency, numFacturas As Integer
    On Error GoTo ErrTrap
    orden1 = 1
    orden2 = 1
    ...
    If CreaComprobanteIngresoAuto(i) Then
        ...
        Set pc = mobjGNCompAux.Empresa.RecuperaPCProvCli(Right("000000" + grd.TextMatrix(fila, 5), 6))
        If Not pc Is Nothing Then
            gobjMain.objCondicion.fecha2 = DateAdd("yyyy", 1, Date)
            gobjMain.objCondicion.PorCobrar = True
            gobjMain.objCondicion.Proveedor = False
            gobjMain.objCondicion.Cliente = True
            gobjMain.objCondicion.CodPC1 = pc.CodProvCli
            gobjMain.objCondicion.CodPC2 = pc.CodProvCli
            Set rs = gobjMain.EmpresaActual.ConsTSPorCobrarPagarGarantes
            If rs.RecordCount > 0 Then
                Set gc = gobjMain.EmpresaActual.RecuperaGNComprobante(rs.Fields(3))
            End If
...
'        mobjGNCompAux.Descripcion = "Por Pago en la: " & fcbBanco.KeyText & ", estudiante: " & grd.TextMatrix(fila, 8)
                ...
                'PAGO
                saldo = grd.ValueMatrix(fila, 28)
                For i = 0 To rs.RecordCount - 1
                    Set gc = gobjMain.EmpresaActual.RecuperaGNComprobante(rs.Fields(3))
                    ix = mobjGNCompAux.AddPCKardex
                    mobjGNCompAux.PCKardex(ix).Haber = rs.Fields(11) 'grd.ValueMatrix(fila, 28)
                    mobjGNCompAux.PCKardex(ix).CodProvCli = gc.CodClienteRef
                    mobjGNCompAux.PCKardex(ix).codforma = gc.PCKardex(1).codforma
                    mobjGNCompAux.PCKardex(ix).idAsignado = gc.PCKardex(1).id
                    mobjGNCompAux.PCKardex(ix).NumLetra = gc.CodTrans & " " & gc.numtrans
                    mobjGNCompAux.PCKardex(ix).FechaEmision = gc.FechaTrans
                    mobjGNCompAux.PCKardex(ix).CodVendedor = gc.CodVendedor
                    mobjGNCompAux.PCKardex(ix).Orden = i
                    saldo = saldo - rs.Fields(11)
                    If saldo <> 0 Then
                        rs.MoveNext
                    End If
                Next i
...
'            sst1.Tab = 0
    ...
    '    Me.caption = "Transacción " & mobjGNCompAux.codTrans & " " & mobjGNCompAux.NumTrans
        ...
        grd.TextMatrix(fila, 46) = mobjGNCompAux.CodTrans & " " & mobjGNCompAux.numtrans
        grd.TextMatrix(fila, 47) = mobjGNCompAux.TransID
        GrabarTransIngresoAutoJEPNew = True
        Set ivb = Nothing
        Set pc = Nothing
    End If
    Exit Function
